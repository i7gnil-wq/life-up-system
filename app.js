(function(){
  ...
  function coinIconSvg(){
    return '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="9"/><circle cx="12" cy="12" r="6" opacity="0.45"/><path d="M12 8.3l1.15 2.55 2.55 1.15-2.55 1.15L12 15.7l-1.15-2.55L8.3 12l2.55-1.15L12 8.3Z" fill="currentColor" stroke="none"/></svg>';
  }
  ...
  function renderStoreTaskHistory(){
    ...
    el.innerHTML = items.map(item => {
      ...
      return `<div class="log-item"><div class="left"><div class="diff-chip" style="color:${chipColor}; border:1px solid ${chipBorder};">${chip}</div><div class="info"><div class="t">${escapeHtml(item.name)}</div><div class="d">${escapeHtml((item.trackName ? item.trackName + ' · ' : '') + (item.desc || item.diffName || ''))}</div></div></div><div class="meta"><div class="xp" style="${item.xp < 0 ? 'color:#d97757' : ''}">${item.xp >= 0 ? '+' : '−'}${Math.abs(item.xp)} XP</div><div class="time">${timeStr}</div></div></div>`;
    }).join('');
  }
  ...
  // ---------- luxury store ----------
  function storeItemHtml(item, coins){
    const affordable = coins >= item.cost;
    const disabled = !affordable;
    let label = affordable ? 'شراء' : 'رصيد غير كافي';
    return `
  ...
  function renderStore(){
    ...
    ensureStore();
    const coins = availableCoins();
    grid.innerHTML = STORE_ITEMS.map(it => storeItemHtml(it, coins)).join('');
    if(histEl){
  ...
  document.getElementById('start-btn').addEventListener('click', async () => {
    ...
    const newCode = code();
    state = { name, code:newCode, xp:0, tasks:[], pending:[], mainTasks:[], mainTasksDay:mainDayKey(), taskListMode:'main', penalties:[], skills:[], syndromes:[], achievements:[], habits:[], coinsSpent:0, purchases:[] };
    ensureSkills();
    ensureSyndromes();
    await saveState();
